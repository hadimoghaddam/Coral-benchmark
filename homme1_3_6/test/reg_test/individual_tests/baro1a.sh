...
if ( $?HOMME_OUT ) then
	set out_dir = $HOMME_OUT/preqx/baro1a
	echo Found \$HOMME_OUT, output will be stored in $out_dir
else
	set out_dir = ~/scratch1/reg-test/preqx/baro1a
...
set CFG = "--enable-blas --enable-lapack --with-netcdf=$NETCDF_PATH --with-pnetcdf=$PNETCDF_PATH NP=8 PLEV=20"
if ( -e config.status ) then
	...
	if ( ( $NP == 8 ) && ( $PLEV == 20 ) && ( $PIO != 1 ) ) then
		echo Already configured with NP = 8, PLEV = 20, and PIO_INTERP...
...
cp $start_dir/$namelist_dir/baro1a.nl $out_dir/input.nl
cd $out_dir
rsync -a $start_dir/../vcoord/*20* ./vcoord
rm -f baro1a.ncl
ln -s $start_dir/ncl/baro1a.ncl ./
...
date
ncl baro1a.ncl | tee -a preqx.out
