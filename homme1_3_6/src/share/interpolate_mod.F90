...
module interpolate_mod
  ...
  use coordinate_systems_mod, only : spherical_polar_t, cartesian2d_t, cartesian3D_t, sphere2cubedsphere, &
	spherical_to_cart, cubedsphere2cart, distance, ref2sphere_double
  ...
  logical, public, save :: interpolate_analysis(8) =&
...
!          endif
  ...
  subroutine setup_latlon_interp(elem,interpdata,par)
    ...
    if (missing_pts>0) then
       ...
       if(par%masterproc) then
          write(iulog,"(3A,I4,A,I7,a,i5)")"Error:",__FILE__," ",__LINE__," count_total:"&
	,count_total," missing:",missing_pts
       end if
       call syncmp(par)
