...
subroutine prim_advance_si(elem, nets, nete, cg, blkjac, red, &
       ...
       use diffusion_mod, only :  prim_diffusion
       use physical_constants, only : kappa, rrearth, rgas, cp, cpwater_vapor, rwater_vapor
  ...
  if (hypervis_order == 2) then
     ...
     do ic=1,hypervis_subcycle
        ...
        do ie=nets,nete
           ...
           ! comptue mean flux
           if (nu_p>0) then
              elem(ie)%derived%psdiss_ave(:,:)=elem(ie)%derived%psdiss_ave(:,:)+&
	eta_ave_w*elem(ie)%state%ps_v(:,:,nt)/hypervis_subcycle
              elem(ie)%derived%psdiss_biharmonic(:,:)=elem(ie)%derived%psdiss_biharmonic(:,:)+&
	eta_ave_w*pstens(:,:,ie)/hypervis_subcycle
           endif
           nu_scale=1
#if (defined ELEMENT_OPENMP)
  ...
  if (hypervis_order == 2) then
     ...
     do ic=1,hypervis_subcycle
        ...
        do ie=nets,nete
           ...
           ! comptue mean flux
           if (nu_p>0) then
              elem(ie)%derived%psdiss_ave(:,:)=elem(ie)%derived%psdiss_ave(:,:)+eta_ave_w*&
	elem(ie)%state%ps_v(:,:,nt)/hypervis_subcycle
              elem(ie)%derived%psdiss_biharmonic(:,:)=elem(ie)%derived%psdiss_biharmonic(:,:)+&
	eta_ave_w*pstens(:,:,ie)/hypervis_subcycle
           endif
           nu_scale=1
...
!$OMP BARRIER
  ...
  real (kind=real_kind), dimension(np,np,nlev)   :: vgrad_ps ! v.grad(lnps) 
