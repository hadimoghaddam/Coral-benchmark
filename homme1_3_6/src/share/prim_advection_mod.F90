...
!$omp    private(zaccbotV,rho_barU,rho_barV)
       ...
       do ij = 1, npsq
          ...
          i = ij - (j-1)*np
          do k=1,nlev
            dp(k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + ( hvcoord%hybi(k+1) -&
	 hvcoord%hybi(k) )*elem(ie)%state%ps_v(i,j,np1)
          ...
          do jk = 1,nlev
            ...
            enddo
            zaccbotU = zaccintegerbU + (za0U(ibotU)*zgamU(jk+1)+(za1U(ibotU)/2)*(zgamU(jk+1)**2)+&
	(za2U(ibotU)/3)*(zgamU(jk+1)**3))*zhdpU(ibotU)
            ...
            enddo
            zaccbotV = zaccintegerbV + (za0V(ibotV)*zgamV(jk+1)+(za1V(ibotV)/2)*(zgamV(jk+1)**2)+&
	(za2V(ibotV)/3)*(zgamV(jk+1)**3))*zhdpV(ibotV)
          ...
          do j=1,np
             ...
             do k=1,nlev
                ...
                if (compute_diagnostics .and. q==1) then
                   elem(ie)%accum%IEvert1_wet(i,j) = elem(ie)%accum%IEvert1_wet(i,j) + (Cpwater_vapor-Cp)*&
	elem(ie)%state%T(i,j,k,n0)*Q_vadv
                endif
#endif	
             enddo
          enddo
  ...
  do ie = nets , nete
    do j = 1 , np
      do i = 1 , np
        ...
        pin(1) = 0.
        do k = 1 , nlev
          dpn = (hvcoord%hyai(k+1) - hvcoord%hyai(k))*hvcoord%ps0 + (hvcoord%hybi(k+1) -&
	 hvcoord%hybi(k))*elem(ie)%state%ps_v(i,j,np1)
