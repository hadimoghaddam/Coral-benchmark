 ...
 integer function create_mpiio(File,fname) result(ierr)
     ...
     if(file%iosystem%ioproc) then
        ...
        case(iotype_pbinary,iotype_direct_pbinary)
           ...
           if(Debug) print *,'OpenFile: io_rank: ',File%iosystem%io_rank,'amode,info: ',&
	amode,File%iosystem%info,'fname: ',fname
 ...
 integer function open_mpiio(File,fname) result(ierr)
     ...
     if(File%iosystem%ioproc) then
        ...
        case(iotype_pbinary,iotype_direct_pbinary)
           ...
           if(Debug) print *, subName,':: io_rank: ',File%iosystem%io_rank,'amode,info: ',&
	amode,File%iosystem%info,'fname: ',fname
           !---------------------------------
           ! set the base file offset to zero
           !---------------------------------
...
!DBG     print *,'TEMPLATE_NAME(write_mpiio_): At the begining of subroutine: ',File%offset
     ...
     call MPI_File_set_view(File%fh,File%offset, iodesc%Write%elemTYPE, iodesc%Write%fileTYPE, &
...
# 159 "iompi_mod.F90.in"
 integer function write_mpiio_real (File,IOBUF,varDesc, iodesc) result(ierr)
	...
	' inside write_mpiio_real offset: ',File%offset
     call MPI_File_set_view(File%fh,File%offset, iodesc%Write%elemTYPE,&
	 iodesc%Write%fileTYPE, 'native',File%iosystem%info,ierr)
	...
	call piodie(_FILE_,__LINE__)
     ...
     call MPI_file_write_all(File%fh,IOBUF,iodesc%Write%n_elemTYPE,&
	iodesc%Write%elemTYPE,fstatus,ierr)
     if(Check.and.ierr/=MPI_SUCCESS) then
...
# 228 "iompi_mod.F90.in"
 integer function read_mpiio_text (File,IOBUF,varDesc, iodesc) result(ierr)
    ...
    call t_startf("pio_read_mpiio_text")
     ...
     if(Debug) print *, subName,':: IAM: ',File%iosystem%io_rank,&
	' read_real8_mpiio: SIZE(IOBUF),n_RelemTYPE: ', &
	...
	File%offset = INT(varDesc%rec-1,kind=PIO_OFFSET)*reclen
     ...
     ! Set file view for distributed array
     !-------------------------------------
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	' before set_view iodesc%Read%fileTYPE: ',iodesc%Read%FileTYPE
     ...
     !-----------------------------
     call MPI_file_read_all(File%fh,IOBUF,iodesc%Read%n_elemTYPE,&
	iodesc%Read%elemTYPE,fstatus,ierr)
     if(Check) call CheckMPIreturn('read_mpiio_text: after call to &
	file_read_all: ',ierr)
     if(Debug) call MPI_get_count(fstatus,iodesc%Read%elemTYPE,cnt,ierr)
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpiio_text: cnt is: ',iodesc%Read%n_elemTYPE, cnt
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpio_text: reclen is: ',reclen
 ...
 integer function read_mpiio_real (File,IOBUF,varDesc, iodesc) result(ierr)
	...
	iodesc%Read%elemTYPE,fstatus,ierr)
     ...
     if(Debug) call MPI_get_count(fstatus,iodesc%Read%elemTYPE,cnt,ierr)
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpiio_real: cnt is: ',iodesc%Read%n_elemTYPE, cnt
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpio_real: reclen is: ',reclen
 ...
 integer function read_mpiio_double (File,IOBUF,varDesc, iodesc) result(ierr)
	...
	iodesc%Read%elemTYPE,fstatus,ierr)
     ...
     if(Debug) call MPI_get_count(fstatus,iodesc%Read%elemTYPE,cnt,ierr)
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpiio_double: cnt is: ',iodesc%Read%n_elemTYPE, cnt
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpio_double: reclen is: ',reclen
 ...
 integer function read_mpiio_int (File,IOBUF,varDesc, iodesc) result(ierr)
	...
	iodesc%Read%elemTYPE,fstatus,ierr)
     ...
     if(Debug) call MPI_get_count(fstatus,iodesc%Read%elemTYPE,cnt,ierr)
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpiio_int: cnt is: ',iodesc%Read%n_elemTYPE, cnt
     if(Debug) print *,_FILE_,__LINE__,'IAM: ',File%iosystem%io_rank,&
	'read_mpio_int: reclen is: ',reclen
...
# 296 "iompi_mod.F90.in"
 subroutine Write_FORTRAN_CntrlWord(File,reclen)
     ...
     datarep = 'native'
     call MPI_File_set_view(File%fh,File%offset,MPI_INTEGER, MPI_INTEGER,&
	 datarep,File%iosystem%info,ierr)
     if(Check) call CheckMPIreturn('write_FORTRAN_CntrlWord: after call to&
	 file_write_set_view: ',ierr)
